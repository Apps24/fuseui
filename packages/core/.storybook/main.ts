...
const config: StorybookConfig = {
  ...
  viteFinal: async (config) => {
    ...
    return mergeConfig(config, {
      ...
      resolve: {
        alias: [
          // @fuse/core points one level up (it IS the current package)
          { find: '@fuse/core',     replacement: path.resolve(__dirname, '../src/index.ts') },
          { find: '@fuse/button',   replacement: pkg('button') },
          { find: '@fuse/icon',     replacement: pkg('icon') },
          { find: '@fuse/input',    replacement: pkg('input') },
          { find: '@fuse/label',    replacement: pkg('label') },
          { find: '@fuse/select',   replacement: pkg('select') },
          { find: '@fuse/checkbox', replacement: pkg('checkbox') },
          { find: '@fuse/card',     replacement: pkg('card') },
          { find: '@fuse/badge',    replacement: pkg('badge') },
          { find: '@fuse/avatar',   replacement: pkg('avatar') },
          { find: '@fuse/skeleton', replacement: pkg('skeleton') },
          { find: '@fuse/toast',    replacement: pkg('toast') },
          { find: '@fuse/modal',    replacement: pkg('modal') },
          { find: '@fuse/alert',    replacement: pkg('alert') },
          { find: '@fuse/spinner',  replacement: pkg('spinner') },
        ],
      },
    });
  },
